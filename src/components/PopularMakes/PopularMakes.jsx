...
export default function PopularMakes({ vehicles = [] }) {
  ...
  const makes = useMemo(() => {
    ...
    vehicles.forEach((v) => {
      if (v.make) {
        map[v.make] = (map[v.make] || 0) + 1;
      }
    });
    return Object.entries(map)
      .sort((a, b) => b[1] - a[1])          // most vehicles first
      .map(([name, count]) => ({ name, count }));
  }, [vehicles]);

  const [activeMake, setActiveMake] = useState(makes[0]?.name || '');

  const filtered = useMemo(
    () => vehicles.filter((v) => v.make === activeMake).slice(0, 4),
    [vehicles, activeMake]
  );
  ...
  return (
    <section className={styles.section} id="popular-makes">
      <div className="container">
        ...
        <div className={styles.tabs}>
          {makes.map((m) => (
            <button
              key={m.name}
              className={`${styles.tab} ${activeMake === m.name ? styles.tabActive : ''}`}
